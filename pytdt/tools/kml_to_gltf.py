...
class OutputMesh:
    ...
    def finish(self, writer):
        ...
        for prim in self.primitives:
            ...
            if indices.max() < 2**8: indices,idx_type    = indices.astype(np.uint8),  GL_UNSIGNED_BYTE
            elif indices.max() < 2**16: indices,idx_type = indices.astype(np.uint16), GL_UNSIGNED_SHORT
            else: indices,idx_type                       = indices.astype(np.uint32), GL_UNSIGNED_INT
            index_bv_id = writer.appendMainBufferAndGetBufferView(indices.tobytes())
